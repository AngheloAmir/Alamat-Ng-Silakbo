...
func _update_environment_and_sun(delta: float) -> void:
	...
	# 1. Rotate Sun Directional Light around sky
	if sun_light:
		var sun_angle_rad: float = (day_time * TAU) - (PI * 0.5)
		sun_light.rotation.x = sun_angle_rad
		var sun_height_factor: float = clampf(sin(day_time * TAU), 0.05, 1.0)
		sun_light.light_energy = lerpf(0.2, 1.1, sun_height_factor)

	# 2. Calculate Time-of-Day Base Sky & Fog Colors
	var target_sky_top: Color = Color(0.35, 0.65, 0.95)
	var target_horizon: Color = Color(0.68, 0.8, 0.92)
	var target_render_far: float = 650.0
	...
	if day_time < 0.25:
		...
		target_render_far = lerpf(650.0, 320.0, t)

	else:
		# --- NIGHT (0.75 - 1.00) ---
		target_sky_top = Color(0.02, 0.04, 0.12)
		target_horizon = Color(0.05, 0.08, 0.18)
		target_render_far = 320.0

	# 3. Apply Season Tint Modifier
	var season_tint: Color = _get_season_tint()
	target_horizon = target_horizon * season_tint

	# 4. Apply Sky & Fog Colors (fog_sky_affect = 0.2 eliminates white frustum light rays!)
	if sky_mat:
		...
		sky_mat.ground_bottom_color = sky_mat.ground_bottom_color.lerp(target_horizon, delta * 2.0)

	env.fog_enabled = true
	env.fog_mode = Environment.FOG_MODE_DEPTH
	env.fog_sky_affect = 0.2 # Low sky affect prevents harsh white cone washouts!
	...
	# 5. Update Camera Render Far Distance for Night Reduction
...
func _get_season_tint() -> Color:
	match current_season:
		...
		Season.SUMMER: return Color(1.15, 1.15, 0.85)
		Season.AUTUMN: return Color(1.2, 0.75, 0.42)
		Season.WINTER: return Color(1.4, 1.45, 1.5)
